...
enum keybind_action {
  ...
  ACTION_PRESELECT_LEFT,
  ACTION_PRESELECT_RIGHT,
  ACTION_PRESELECT_UP,
  ACTION_PRESELECT_DOWN,
  ACTION_SWAP_LEFT,
  ACTION_SWAP_RIGHT,
  ACTION_SWAP_UP,
  ACTION_SWAP_DOWN,
...
enum gesture_action_type {
  GESTURE_ACTION_NONE,
  GESTURE_ACTION_WORKSPACE_NEXT,
  GESTURE_ACTION_WORKSPACE_PREV,
  GESTURE_ACTION_WORKSPACE,
  GESTURE_ACTION_MAXIMIZE,
  GESTURE_ACTION_MINIMIZE,
  GESTURE_ACTION_FULLSCREEN,
  GESTURE_ACTION_CLOSE,
  GESTURE_ACTION_SNAP_LEFT,
  GESTURE_ACTION_SNAP_RIGHT,
  GESTURE_ACTION_TOGGLE_FLOATING,
  GESTURE_ACTION_TOGGLE_MODE,
  GESTURE_ACTION_SPAWN,
  GESTURE_ACTION_EXEC,
};

struct gesture_action {
  enum gesture_action_type type;
  char arg[256];
};

struct gesture_touchpad {
  int fingers;
  enum gesture_direction direction;
  struct gesture_action action;
};

struct gesture_mouse {
  uint32_t button;
  uint32_t modifiers;
  enum gesture_direction direction;
  struct gesture_action action;
...
struct config {
  ...
  // Gestures
  struct gesture_touchpad *gesture_touchpad;
  int gesture_touchpad_count;
  struct gesture_mouse *gesture_mouse;
  int gesture_mouse_count;

  double gesture_swipe_threshold;
  double gesture_pinch_threshold;
  double gesture_mouse_threshold;
