...
void decor_create(struct toplevel *toplevel) {
    ...
    d->shadow = create_shadow_buffer(server->layer_windows, window_width, window_height + h, 
                                      &config.decor.shadow);
    if (d->shadow) {
...
void decor_set_size(struct toplevel *toplevel, int width) {
    if (!config.decor.enabled || !toplevel->decor.tree || !g_global_theme) return;
    ...
    d->shadow = create_shadow_buffer(toplevel->server->layer_windows, width, total_height, &config.decor.shadow);
    if (d->shadow) {
...
void decor_set_position(struct toplevel *toplevel, int x, int y) {
    ...
    struct decoration *d = &toplevel->decor;
    wlr_scene_node_set_position(&d->tree->node, x, y);
    
    if (d->shadow) {
        wlr_scene_node_set_position(&d->shadow->node, x, y);
    }
}
...
enum decor_hit decor_hit_test(struct toplevel *toplevel, double lx, double ly) {
    if (!config.decor.enabled || !toplevel->decor.tree || !g_global_theme) return HIT_NONE;
    ...
    if (y >= 0 && y < h && d->rendered_titlebar) {
        ...
        switch (btn) {
        case BTN_TYPE_CLOSE: return HIT_CLOSE;
        case BTN_TYPE_MAXIMIZE: return HIT_MAXIMIZE;
        case BTN_TYPE_MINIMIZE: return HIT_MINIMIZE;
        default: return HIT_TITLEBAR;
        }
    }
...
void decor_update_hover(struct toplevel *toplevel, double lx, double ly) {
    ...
    if (y >= 0 && y < h) {
        ...
        switch (btn) {
        case BTN_TYPE_CLOSE: close_state = BTN_STATE_HOVER; break;
        case BTN_TYPE_MAXIMIZE: max_state = BTN_STATE_HOVER; break;
        case BTN_TYPE_MINIMIZE: min_state = BTN_STATE_HOVER; break;
        default: break;
        }
    }
